...
@Component
public class CreateDao extends CommonCreate implements CreateFile {
	...
	public void mainLogic(List<Pojo> pojos) throws IOException {
		String readFileToString = FileUtils.readFileToString(new File(ConfigUtil.get("model.dao")));
	...
	public void setOwnData(List<Pojo> pojos) {
		for (Pojo p : pojos) {
			setFileName(p);
			setPackageName(p, ConfigUtil.get("package.dao"));
			setFilePath(p, ConfigUtil.get("package.dao"));
			setFullName(p);
			setImport(p);
		}
	}
	...
	public void setFileName(Pojo p) {
        p.setFileName(p.getPojoName() + "Dao.java");		
	}

	public void setPojoName(Pojo p) {

	}

	public void setFullName(Pojo p) {
		p.setFullName(p.getPackageName() + "." + p.getPojoName() + "Dao");
	}
